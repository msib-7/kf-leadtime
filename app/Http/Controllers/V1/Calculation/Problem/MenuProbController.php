...
use App\Models\V1\xxdash_lt_result_problem;
...
class MenuProbController extends Controller
{
    //
    public function index()
    {
        ...
        return view('v1.calculation.problem.menuprob'); // Tidak perlu compact('results') lagi
    }
    ...
    public function getCalculationData(Request $request)
    {
        if ($request->ajax()) {
            ...
            $query = xxdash_lt_result_problem::query();
    ...
    {
        $lines = xxdash_lt_result_problem::select('prod_line')
            ->distinct()
            ->orderBy('prod_line')
            ->pluck('prod_line');
    ...
    public function getAvailableYears()
    {
        ...
        $years = xxdash_lt_result_problem::select(DB::raw("DISTINCT EXTRACT(YEAR FROM TO_TIMESTAMP(waktu_release, 'MM/DD/YYYY HH24:MI:SS')) as year"))
    ...
    public function getAvailableMonths(Request $request)
    {
        ...
        $query = xxdash_lt_result_problem::select(DB::raw("EXTRACT(MONTH FROM TO_TIMESTAMP(waktu_release, 'MM/DD/YYYY HH24:MI:SS')) as month"))
            ->distinct();
    ...
    public function destroy(Request $request, $lot_number)
    {
        try {
            ...
            $record = xxdash_lt_result_problem::where('lot_number', $lot_number)->first();
    ...
    public function rollback(Request $request, $lot_number)
    {
        try {
            ...
            $recordToRollback = xxdash_lt_result_problem::where('lot_number', $lot_number)->first();
            if (!$recordToRollback) {
                return response()->json([
                    'success' => false,
            ...
            $originalRecord = xxdash_lt_result_excl::where('lot_number', $lot_number)->first();
            if (!$originalRecord) {
    ...
    public function rollbackLineX(Request $request, $lot_number)
    {
        try {
            DB::beginTransaction();
            $recordToUpdate = xxdash_lt_result_problem::where('lot_number', $lot_number)->first();
            if (!$recordToUpdate) {
                return response()->json([
                    'success' => false,
                    'message' => 'Data not found in exclude table'
